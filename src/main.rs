...
fn process_flight(reservation: Flight){
    ...
    println!("Processing flight {} to {} from {}",reservation.airline, reservation.destination, reservation.origin);
    let id_str = format!("{}{}{}", reservation.airline.to_owned(), reservation.destination, reservation.origin);
    reservation.result_gateway.send( FlightResult{id: id_str, accepted: true} );
}
...
fn create_webservice(rate_limit: u32) -> Webservice {
    ...
    Webservice { recv: Arc::new(Mutex::new(wbs_recv)), send: Arc::new(Mutex::new(wbs_send))}
}
...
fn main() {
    ...
    //for line in reader.lines().flatten() {
        ...
        threads.push(thread::spawn(move || {
            ...
            println!("Flight {} has been proccessed with result {}", result.id, result.accepted);
        }));
    //}
