...
fn test_run_program_with_3mixed_reqs_2webservices_100success_rate() {
    ...
    let web_services = load_services(&results_service, hotel,
                                     vec!["american,10,100".to_string(), "italian,10,100".to_string()]).unwrap();
...
fn test_run_program_test_webservice_always_fails() {
    ...
    //creates all web services
    let web_services = load_services(&results_service, hotel,
                                     vec!["american,4,0".to_string(), "italian,1,0".to_string()]).unwrap();
...
fn test_run_program_for_bottleneck() {
    ...
    //creates all web services
    let web_services = load_services(&results_service, hotel,
                                     vec!["american,1,1".to_string()]).unwrap();
...
fn test_run_program_incorrect_airlines() {
    ...
    //creates all web services
    let web_services = load_services(&results_service, hotel,
                                     vec!["american,1,100".to_string()]).unwrap();
...
fn load_services(resultservice: &Arc<ResultService>, hotel : Arc<Webservice>, test_args : Vec<String> ) -> Option<HashMap<String, ScheduleService>>{
